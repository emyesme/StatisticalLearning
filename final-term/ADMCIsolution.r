...
train <- read.csv("/home/emily/Desktop/StatisticalLearning/final-project/ADMCItrain.csv")
test <- read.csv("/home/emily/Desktop/StatisticalLearning/final-project/ADMCItest.csv")
...
nocorr_features <- features_prepro[, !apply(tmp, 2, function(x) any( x>=0.75 & x<=0.99 , na.rm = TRUE))]

ncol(nocorr_features) #18
...
lasso.coef <- predict(lasso.mod, s = lasso.cv$lambda.min, type="coefficients")[1:8, ]
...
# discard features?
elastic.coef <- predict(elastic.model, s = elastic.cv$bestTune$lambda, type = "coefficients")[1:6, ]
