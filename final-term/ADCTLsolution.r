'''
  ...
  You will need to train a classification model for each of the problems and test it on the corresponding test dataset. Your task is to choose the classifier that achieves the best performance on the test datasets. 
  You will receive three train datasets (ADCTLtrain.csv, ADMCItrain.csv, MCICTLtrain.csv) and three corresponding test datasets (*test.csv). These include different numbers of observations (between 164 and 172, one for each row) and several independent variables (between 64 and 594, along the columns). The first column (‘ID’) indicates the ID of each observation. The last column (‘Label’) of the train datasets includes the class the observations belong to (‘AD’, ‘MCI’, or ‘CTL’, depending on the training set). 
  For each of the three binary problems, your submission will consist of: 
  1. Two RData files whose name are formatted as:  
  StudentRegistrationNumber_FamilyName_*res.Rdata 
  StudentRegistrationNumber_FamilyName_*feat.Rdata 
  ...
  2. A presentation in PDF with up to 6 pages, in which you describe how you obtained the model. Please, observe that it is NOT mandatory to choose the same classification model for the three different binary problems; just choose the one that leads to the most promising results. 
  3. The R macro (script) used to obtain the results, named: 
  StudentRegistrationNumber_FamilyName_solution.R. 
  For each problem, the results will be ranked according to AUC (e.g., auc R function from pROC) and MCC (e.g., mcc R function from mltools).
'''

set.seed(42)

# reading contents of csv file
train <- read.csv("/home/emily/Desktop/StatisticalLearning/final-project/ADCTLtrain.csv")
test <- read.csv("/home/emily/Desktop/StatisticalLearning/final-project/ADCTLtest.csv")
...
#features
features <- train[2:430]

# MRI features are normalised, GE not -> normalise GE
...
# generating train and validation for lasso and elasticnet
...
lasso.cv <- cv.glmnet(x[train_train, ], y[train_train], alpha = 1, family="binomial", type.measur='auc')
...
elastic.cv <- train(
                    label ~. ,
                    data = train_regsubset[train_train, ],
                    method = "glmnet",
...
# create a model with the final best values of cross validation
elastic.model <- glmnet(x, y, alpha = elastic.cv$bestTune$alpha, lambda=elastic.cv$bestTune$lambda)

# predict in the validation set created previously
...
# discard features?
elastic.coef <- predict(elastic.model, s = elastic.cv$bestTune$lambda, type = "coefficients")[1:39, ]
elastic.coef <- elastic.coef[ elastic.coef != 0]
length(elastic.coef)
# no features discarded but in lasso yes, no significance in error difference and lasso discard features
...
train_features <- train_regsubset[, c(lasso.predictors)]
